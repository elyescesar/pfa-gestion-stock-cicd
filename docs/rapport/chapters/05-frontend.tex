...
En production, le build utilise \texttt{VITE\_API\_URL=https://api.pfa.elyes.dev} : les appels REST ciblent directement le sous-domaine API.
...
\texttt{nginx.conf} configure le fallback SPA (\texttt{try\_files}). En Compose, un proxy \texttt{/api/} optionnel existe ; en production, le navigateur appelle l'API sur un hôte distinct.
