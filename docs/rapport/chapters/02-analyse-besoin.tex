...
\begin{enumerate}
  ...
  \item \textbf{Deux chemins de déploiement} : Compose local (\texttt{make dev}) et k3s sur VPS dédié (CD GitHub Actions).
  \item \textbf{Automatisation} : \texttt{make demo} en local ; push sur \texttt{main} pour la production.
  \item \textbf{Qualité logicielle} : tests, lint, smoke tests (\texttt{make verify}).
  \item \textbf{Isolation production} : VPS PFA séparé de tout hébergement Coolify client.
\end{enumerate}
