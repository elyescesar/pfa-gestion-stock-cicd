...
Le déploiement couvre le \textbf{développement local} et la \textbf{production sur VPS dédié} (TLS, secrets GitHub). Les menaces principales sont : accès non autorisé à l'API, fuite de secrets dans Git, injection SQL (mitigée par ORM), et configuration CORS trop permissive.
...
\begin{itemize}
  ...
  \item Contrôles de rôle sur DELETE (vérification \texttt{role == admin}).
\end{itemize}
...
\texttt{CORS\_ORIGINS} liste les origines autorisées (dev : \texttt{localhost:3000} ; production : \texttt{https://pfa.elyes.dev}).

\section{Réseau Kubernetes}

Les Services clusterIP ne sont pas exposés sauf via Ingress (80/443). L'API n'écoute pas sur les ports hôte 8000, 6001 ou 6002.
...
\begin{itemize}
  \item curl sur \texttt{/health}, login, liste produits, page web (stack dev locale).
\end{itemize}
